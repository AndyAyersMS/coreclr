...
extern "C" void __cdecl assertAbort(const char* why, const char* file, unsigned line)
{
    ...
    const char* phase     = PhaseNames[env->compiler->mostRecentlyActivePhase];
    if (env->compiler)
    {
        _snprintf_s(buff, BUFF_SIZE, _TRUNCATE, "Assertion failed '%s' in '%s' during '%s' (IL size %d)\n", why,
                    env->compiler->info.compFullName, phase, env->compiler->info.compILCodeSize);
        msg = buff;
    }
    ...
    if (Compiler::compJitFuncInfoFile != nullptr)
    {
        fprintf(Compiler::compJitFuncInfoFile, "%s - Assertion failed (%s:%d - %s) during %s\n",
                (env == nullptr) ? "UNKNOWN" : env->compiler->info.compFullName, file, line, why, phase);
    }
#endif // FUNC_INFO_LOGGING
